...
export class OrderSyncWorkflow extends WorkflowEntrypoint<Env, OrderSyncPayload> {
  async run(event: WorkflowEvent<OrderSyncPayload>, step: WorkflowStep): Promise<void> {
    const { eventId, source, sourceOrderId } = event.payload

    if (!sourceOrderId) {
      await markWebhookSkipped(step, this.env.DB, eventId, "no source order id on event")
      return
    }
    ...
    log.info({ event: "order.synced", source, source_order_id: sourceOrderId, order_status: order.rawStatus })
